...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const handleWhatsAppInquiry = (e: React.MouseEvent) => {
    ...
    const msg =
      language === 'en'
        ? `Hello ART FASHION Cotonou 👋\nI am interested in ordering: *${product.name}* (${formatFCFA(product.price)}).\nIs size ${selectedVariant.size} (${selectedVariant.color}) available for delivery in Cotonou?`
        : `Bonjour ART FASHION Cotonou 👋\nJe souhaite commander le modèle : *${product.name}* (${formatFCFA(product.price)}).\nEst-il disponible en taille ${selectedVariant.size} (${selectedVariant.color}) pour livraison à Cotonou ?`;
